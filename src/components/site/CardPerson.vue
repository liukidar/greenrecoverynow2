<template>
  <div class="cmp-card-person no-padding z-depth-3 white" style="position: relative;">
    <div class="cmp-profile-pic-circle"><person :img="'static/imgs/' + img.file" :position="img.offset"></person></div>
    <layer>
        ...
        <div class="card-person">
          <div class="card-padding">
            ...
            <div class="left-align">
              ...
              <span class="flow-text" style="padding: 1rem;" @click="(e) => { $el.children[0].classList.add('transparent'); return layer.set(e, 'contacts', 'bkg-main-color-dark') }"><span class="slash"></span><i style="cursor:pointer;" class="text-important text-grey text-darken-4">contacts</i></span>
            </div>
          </div>
        </div>
      </div>
      <paper-container class="text-important card-padding white-text" style="height:100%;" slot="contacts" slot-scope="layer">
        <template slot="paper-before">
          ...
          <img class="z-depth-4 cmp-profile-pic hide-on-med-and-down" :src="'static/imgs/' + img.file">
        ...
        <i class="material-icons close-arrow btn-hm waves-effect" @click="(e) => { $el.children[0].classList.remove('transparent'); return layer.set(e, null, 'white') }">arrow_back</i>
      </paper-container>
...
.cmp-profile-pic-circle {
  opacity: 1;
  transition: opacity .5s;
  position: relative;
  z-index: 999;
}
.cmp-profile-pic-circle.transparent {
  opacity: 0;
}
...
.cmp-card-person.right-side .cmp-profile-pic {
  position: absolute;
  right: 5%;
  bottom: 10%;
  width: 256px;
